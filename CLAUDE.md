...
This repo publishes a daily-refreshed static site showing the zh_CN translation status of the Linux kernel documentation. The status checker itself — `checktransupdate.py` — lives upstream in the kernel tree at `tools/docs/checktransupdate.py`; this repo does NOT keep a local copy. What this repo does contain:
...
- A harness ([gen_todolist.sh](gen_todolist.sh)) that clones the kernel tree and invokes the in-tree checker to regenerate the tracked [TODO_LIST](TODO_LIST).
...
Note: [gen_todolist.sh](gen_todolist.sh) clones `git://git.lwn.net/linux.git --branch docs-next` into `./linux/` (gitignored) and then invokes `./linux/tools/docs/checktransupdate.py` directly — there is no local copy to override it.
...
The `checktransupdate.py` script itself is not mirrored locally; edits to the script flow upstream to `tools/docs/checktransupdate.py` via the normal kernel patch process, and the CI workflow pulls the latest version from LWN's `docs-next` branch at build time.
