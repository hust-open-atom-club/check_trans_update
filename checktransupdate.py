...
def get_origin_from_trans(origin_path, t_from_head):
    """Get the latest origin commit that the translation was likely based on.

    Prefer commits reachable from the translation commit (same branch
    history). If the origin file is not in the translation's ancestry --
    e.g. when the English docs were merged into the branch AFTER the
    translation was authored, as happens with multi-tree merge flows --
    fall back to searching from HEAD for the latest origin commit whose
    author_date is <= the translation's author_date.
    """
    o_from_t = get_latest_commit_from(origin_path, t_from_head["hash"])
    if o_from_t is None:
        # Origin file not in translation's ancestry; search from HEAD.
        o_from_t = get_latest_commit_from(origin_path, "HEAD")
